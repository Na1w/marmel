...
- 'QueueAndContinue': ONLY select this if the new instruction is a completely independent FUTURE ACTIONABLE TASK OR GOAL that the orchestrator must execute after current active subtasks finish (e.g. 'när du är klar, implementera även X', 'sen kan du fixa Y').
  CRITICAL: NEVER select 'QueueAndContinue' for questions, status inquiries, greetings, or conversational remarks (e.g. 'hur går det?', 'vad gör du nu?', 'hur lång tid har det tagit?', 'har testerna passerat?'). All questions and inquiries MUST be answered via 'RespondDirectly' (or 'DelegateTask') and must NEVER be queued to the orchestrator.
...
- 'RespondDirectly': Select this whenever the user asks ANY question, status check, inquiry, greeting, or remark (e.g. 'hur går det?', 'vad gör du nu?', 'vad händer?', 'hur lång tid har det tagit?', 'när blev du klar med X?', 'hur många valideringsrundor?', 'vad sa validatorn?').
