...
- 'RespondDirectly': Select this whenever the user asks for a status check (e.g. 'what is happening right now?', 'what is the current status?', 'how is it going?', 'how many validation rounds has it done?', 'what was the validator feedback?'), general greetings, high-level conversational questions, or status inquiries that can be answered using the provided context ('Orchestrator Status', 'Execution Plan Progress Breakdown', 'Active Execution Plan', 'Active Subtasks'). Summarize clearly and factually what the active subagents and validator are currently doing, including implementation turns and validator feedback if asked. If answering accurately requires inspecting the filesystem or running code commands that the subagents aren't already doing, use 'DelegateTask'.
